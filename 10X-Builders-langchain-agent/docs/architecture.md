...
4. The agent selects and executes the appropriate tool based on the prompt:
   - `calculator`
   - `current_time`
   - `flight_search` (SerpApi Google Flights; requiere `SERPAPI_API_KEY` en `env.local`)
...
- `src/config/env.ts`
  - Loads `env.local`.
  - Validates environment variables with `zod` (incluye `SERPAPI_API_KEY` opcional para vuelos).
- `src/agent/model.ts`
